...
float sphere_mass(float radius, float density) { return 4.0 / 3.0 * PI * radius * radius * radius * density; }

State::State(std::size_t spheres, std::size_t n, std::size_t m, float spacing, float water_height,
    ...
    : _n(n), _m(m), _spacing(spacing), _sphere_centers(spheres * 3, 0.0), _water_heights(n * m, water_height),
      ...
      _sphere_masses(spheres), _sphere_body_heights(spheres * n * m, 0.0), _neighbour_sums(n * m, 0.0) {
  for (std::size_t x = 0; x < _n; ++x) {
    for (std::size_t z = 0; z < _m; ++z) {
      _water_xzs.push_back(x * _spacing - _spacing * (_n - 1) / 2);
      _water_xzs.push_back(z * _spacing - _spacing * (_m - 1) / 2);
    }
  }
  for (std::size_t i = 0; i < spheres; ++i)
