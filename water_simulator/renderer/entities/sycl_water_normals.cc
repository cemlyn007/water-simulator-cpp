...
void update_water_normals(std::vector<float> &vertex_normals, std::vector<float> &face_normals,
  ...
  const std::size_t null_face_index = (indices.size() / 3) - 1;
  const std::size_t vertex_size = vertex_normals.size() / 3;
  const std::size_t max_face_index = (resolution - 1) * (resolution - 1) * 2;
  constexpr std::size_t max_faces_per_vertex = 6;

  static std::vector<float> vertex_face_indices(vertex_normals.size() * max_faces_per_vertex, null_face_index);
  static std::vector<std::size_t> vertex_face_counts(vertex_normals.size(), 0);
  ...
  if (face_normals.size() != (resolution - 1) * (resolution - 1) * 2 * 3 + 3) {
    ...
    face_normals.push_back(0.0);
    face_normals.push_back(0.0);
    for (std::size_t face_index = 0; face_index < indices.size() / 3; ++face_index) {
      for (std::size_t offset = 0; offset < 3; ++offset) {
        const std::size_t vertex_index = indices[face_index * 3 + offset];
        const std::size_t space = vertex_face_counts[vertex_index];
  ...
  sycl::buffer<std::size_t, 1> d_vertex_face_counts(vertex_face_counts.data(),
                                                    sycl::range<1>(vertex_face_counts.size()));
  ...
  auto event = queue.submit([&](sycl::handler &handler) {
    ...
    handler.parallel_for(sycl::range<1>(max_face_index), [=](sycl::id<1> index) {
      const std::size_t face_index = index;
      const std::size_t i = indices_acc[face_index * 3];
      const std::size_t j = indices_acc[face_index * 3 + 1];
      const std::size_t k = indices_acc[face_index * 3 + 2];
  ...
  queue.submit([&](sycl::handler &handler) {
    ...
    handler.parallel_for(sycl::range<1>(vertex_size), [=](sycl::id<1> index) {
      ...
      float y = 0.0;
      float z = 0.0;
      const std::size_t faces = vertex_face_counts_acc[vertex_index];
      for (std::size_t offset = 0; offset < max_faces_per_vertex; ++offset) {
        const std::size_t face_index = vertex_face_indices_acc[vertex_index * max_faces_per_vertex + offset];
